...
class NB::ApiClient
  ...
  def get_page_resource_index_with_id(resource, id)
...
class NB::CsvHandler
  ...
  def retrieve_blog_post_headers(blog_id)
    @client.get_page_resource_index_with_id(:blog_posts, blog_id)['results'].first.keys
  end
  ...
  def create_blog_post_csv(blog_id)
    CSV.open("#{@client.slug}_blog_post_api_index.csv", 'w') do |csv|
      ...
      request = @client.get_page_resource_index_with_id(:blog_posts, blog_id)
